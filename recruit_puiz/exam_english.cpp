...
QuestionList CreateEnglishWordExam()
{
	const struct
	{
		...
		{ "collaboration", "コラボレーション", "強力、共同事業、共同作業"},
		{ "tax", "タックス", "税金"},
		{ "stock", "ストック", "在庫、株券"},
		{ "product", "プロダクト", "製品"},
		{ "booking", "ブッキング", "予約、帳簿への記入"},
		{ "weight", "ウェイト", "重さ"},
		{ "compliance", "コンプライアンス", "法令・社会的規模を守る"},
		{ "receipt", "レシート", "領収書"},
	};
	...
	case 2:
		...
		{
			const auto& e = data[indices[i]];
			questions.push_back({ string(e.word) + "の読み方をカタカナで答えよ", e.reading });
		}
		break;
...
QuestionList CreateEnglishPhraseExam()
{
	const struct
	{
		...
		{ "both [A] and [B]", "[A]と[B]の両方とも",
		  "This bag is both quality and cheap.", "このかばんは品質が良い上に安い。",
		  2, { 3, 5 }},
		{ "either [A] or [B]", "[A] か [B]のどちらか",
		  "Mom says that you can have either a dog or a cat.", "母は、犬か猫のどちらかなら買っても良いと言っている。",
		  2, { 6, 9 }},
